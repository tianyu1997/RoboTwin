...
class explore_2cabinets(Base_Task):
    ...
    def load_actors(self):
        ...
        if n==0:
            self.opposite_cabinet = self.cabinets[1]
        else:
            self.opposite_cabinet = self.cabinets[0]
    ...
    def play_once(self):
        ...
        for _ in range(2):
            self.move(self.move_by_displacement(arm_tag=arm_tag.opposite , x=-self.x))
        self.move(self.grasp_actor(self.opposite_cabinet, arm_tag=arm_tag,pre_grasp_dis=0.05))
        ...
        # for _ in range(3):
        #     self.move(self.move_by_displacement(arm_tag=arm_tag.opposite, z=-0.075))
        
        # self.open_gripper(arm_tag.opposite , pos=1.0)
        # for _ in range(2):
        #     self.move(self.move_by_displacement(arm_tag=arm_tag.opposite, z=0.1))
        # self.move(self.move_to_pose(arm_tag=arm_tag.opposite , target_pose=pose))
